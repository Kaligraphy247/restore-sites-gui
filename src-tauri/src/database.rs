use crate::models::{Database, CollectionRecord};
use crate::utils::get_data_dir;
use chrono::Utc;
use std::fs;
use tracing::{debug, info, warn, instrument};

const DB_FILE_NAME: &str = "projects.json";
...
impl JsonStore {
    ...
    pub fn insert(&self, mut record: CollectionRecord) -> Result<CollectionRecord, Box<dyn std::error::Error>> {
    ...
    #[instrument]
    pub fn get_by_id(&self, id: u64) -> Result<Option<CollectionRecord>, Box<dyn std::error::Error>> {
        let database = self.load()?;
    ...
    #[instrument]
    pub fn search_by_name(&self, query: &str) -> Result<Vec<CollectionRecord>, Box<dyn std::error::Error>> {
        let database = self.load()?;
