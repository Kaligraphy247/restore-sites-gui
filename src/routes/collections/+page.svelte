<script lang="ts">
    ...
    import { ArchiveRestore, ExternalLink, Calendar, Globe } from "@lucide/svelte";
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="font-semibold text-2xl text-neutral-800 dark:text-neutral-100">Collections</h2>
        <div class="text-sm text-neutral-500 dark:text-neutral-400">
            {data.collections.length} collection{data.collections.length !== 1 ? 's' : ''}
        </div>
    </div>

    <div class="space-y-3 h-[450px] overflow-y-auto pr-2">
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <div class="flex items-center gap-2 ml-4">
                            <button 
                                ...
                                class="flex items-center gap-1 px-3 py-1.5 text-sm bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors duration-200"
                    ...
                    <div class="space-y-1.5">
                        {#each collection.sites.slice(0, 3) as site}
                            <div class="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-700/50 rounded px-2 py-1.5">
                                <div class="w-2 h-2 bg-neutral-400 dark:bg-neutral-500 rounded-full flex-shrink-0"></div>
                                <span class="font-medium truncate flex-1">{site.title}</span>
                                <span class="text-xs text-neutral-400 dark:text-neutral-500 truncate max-w-[120px]">
                        ...
                        {#if collection.sites.length > 3}
                            <div class="text-xs text-neutral-400 dark:text-neutral-500 text-center py-1">
        ...
        {#if data.collections.length === 0}
            <div class="text-center py-12 text-neutral-500 dark:text-neutral-400">
                ...
                <p class="text-sm">Create your first collection from the home page</p>
            </div>
        {/if}
    </div>
</div>
